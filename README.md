...
Public behavior tests were developed in individual red/green cycles, starting with safe dry run. They use in-memory IO, controlled time and targeted boundary faults. Real-process tests verify crash/restart behavior. Windows CI exercises a disposable SMB share, outage/restart, sharing locks and junctions. See the [Windows production runbook](docs/PRODUCTION-RUNBOOK.md) for deployment, audit storage and remaining customer acceptance.
