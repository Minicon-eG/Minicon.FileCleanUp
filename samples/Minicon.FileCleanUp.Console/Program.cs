...
var recovery = args.Contains("--audit-list")
    || args.Contains("--audit-ack");
var builder = Host.CreateApplicationBuilder(new HostApplicationBuilderSettings { Args = recovery ? [] : args, ContentRootPath = AppContext.BaseDirectory });
...
var logConfiguration = new LoggerConfiguration().MinimumLevel.Information().Enrich.FromLogContext().Enrich.WithProperty("Application", "Minicon.FileCleanUp.Console").Enrich.WithProperty("MachineName", Environment.MachineName).WriteTo.Console();
if (builder.Configuration["Seq:ServerUrl"] is { Length: > 0 } seq)
{
    logConfiguration.WriteTo.Seq(
        seq,
        apiKey: builder.Configuration["Seq:ApiKey"],
        bufferBaseFilename: Path.Combine(stateDirectory, "seq-buffer"));
}
...
try
{
    ...
    try
    {
        applicationLock = new FileStream(
            Path.Combine(stateDirectory, "cleanup.lock"),
    ...
    {
        Log.Warning(ex, "{EventType}: state directory lock unavailable", "CleanupNotStarted");
        return 3;
    }
    ...
    using (var host = builder.Build())
    {
        if (recovery)
        {
            ...
            if (args.Contains("--audit-list"))
            {
                System.Console.WriteLine(JsonSerializer.Serialize(journal.GetPending(), new JsonSerializerOptions { WriteIndented = true }));
            }
            else
            {
                string Value(string key)
                {
                    ...
                    return index >= 0
                        && index + 1 < args.Length ? args[index + 1] : throw new ArgumentException($"Missing {key}");
                }
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(
                Path.Combine(stateDirectory, $"{result.RunId}.json"),
                JsonSerializer.Serialize(result, new JsonSerializerOptions { WriteIndented = true }));
            return result.Status switch
            {
                CleanupStatus.Succeeded => 0,
                CleanupStatus.InvalidConfiguration => 2,
                CleanupStatus.Limited => 4,
                CleanupStatus.Cancelled => 5,
                _ => 1
            };
        }
        finally
...
{
    Log.Error(ex, "{EventType}: host failed", "CleanupHostFailed");
    return 1;
}
finally
{
