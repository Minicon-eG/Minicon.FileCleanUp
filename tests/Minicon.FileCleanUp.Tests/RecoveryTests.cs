...
public class RecoveryTests
{
    ...
    {
        var fs = new MockFileSystem(); var dir = Path.Combine(Path.GetTempPath(), "recovery"); fs.Directory.CreateDirectory(dir);
        var operation = Guid.NewGuid();
        using (var journal = new LocalAuditJournal(dir, fs)) journal.Append(new() { Type = "DeleteIntent", Path = "data/file", OperationId = operation });
