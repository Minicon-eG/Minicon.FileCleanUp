...
public class AuditTests
{
    ...
    public void Removing_an_initialized_journal_must_not_silently_reset_pending_protection()
    {
        ...
        var dir = Path.Combine(Path.GetTempPath(), "audit-loss"); fs.Directory.CreateDirectory(dir);
    ...
    {
        var fs = new System.IO.Abstractions.TestingHelpers.MockFileSystem();
        var dir = Path.Combine(Path.GetTempPath(), "audit-truncated"); fs.Directory.CreateDirectory(dir);
        using (var journal = new LocalAuditJournal(dir, fs))
            journal.Append(new() { Type = "DeleteIntent", OperationId = Guid.NewGuid(), Path = "old.csv" });
    ...
    {
        var fs = new System.IO.Abstractions.TestingHelpers.MockFileSystem();
        var dir = Path.Combine(Path.GetTempPath(), "audit-rotation"); fs.Directory.CreateDirectory(dir);
        ...
        using (var journal = new LocalAuditJournal(dir, fs, maxSegmentBytes: 1024))
        {
            ...
            for (var i = 0; i < 20; i++) journal.Append(new() { Type = "RunStarted", Reason = new string('x', 300) });
        }
    ...
    {
        var fs = new System.IO.Abstractions.TestingHelpers.MockFileSystem();
        var dir = Path.Combine(Path.GetTempPath(), "audit-large"); fs.Directory.CreateDirectory(dir);
        using (var journal = new LocalAuditJournal(dir, fs))
        {
