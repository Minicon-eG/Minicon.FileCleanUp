...
public class RealFileSystemTests
{
    ...
    public async Task Real_file_system_deletes_only_old_files_and_preserves_root()
    {
        ...
        try
        {
            var root = Path.Combine(work, "data");
            var journalPath = Path.Combine(work, "journal");
            Directory.CreateDirectory(root);
            Directory.CreateDirectory(journalPath);
            for (var i = 0; i < 100; i++)
            {
                var file = Path.Combine(root, $"old-{i}.csv");
                await File.WriteAllTextAsync(file, "old");
                File.SetLastWriteTimeUtc(file, new DateTime(2020, 1, 1, 0, 0, 0, DateTimeKind.Utc));
            }

            var recent = Path.Combine(root, "recent.csv");
            await File.WriteAllTextAsync(recent, "keep");
            var now = new DateTimeOffset(2026, 9, 14, 2, 0, 0, TimeSpan.Zero);
