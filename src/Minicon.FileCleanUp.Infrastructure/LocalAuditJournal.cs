...
public sealed class LocalAuditJournal : ICleanupAuditJournal
{
        ...
        long maxSegmentBytes = 64 * 1024 * 1024)
    {
        ...
        sessionLock = fileSystem.FileStream.New(
            fileSystem.Path.Combine(directory, "audit.lock"),
            FileMode.OpenOrCreate,
            FileAccess.ReadWrite,
            FileShare.None);
        try
        {
            var initializedBefore = fileSystem.File.Exists(marker)
                || fileSystem.File.Exists(checkpointPath);
            ...
            var checkpoint = initializedBefore ? System.Text.Json.JsonSerializer.Deserialize<Checkpoint>(fileSystem.File.ReadAllText(checkpointPath)) ?? throw new IOException("Invalid audit checkpoint.") : new Checkpoint(0, previousHash);
            var checkpointVerified = checkpoint.Sequence == 0
            ...
            void Replay(FileSystemStream input)
            {
                while (input.Position < input.Length)
                {
                    ...
                    if (length is < 1 or > 16_777_216)
                    {
                        throw new IOException("Invalid audit frame length.");
                    }
                    ...
                    if (frame.Sequence != sequence + 1
                        || frame.PreviousHash != previousHash
                        || frame.Hash != Hash(frame.Sequence, frame.PreviousHash, frame.Payload))
                    {
                    ...
                    {
                        checkpointVerified = previousHash == checkpoint.Hash;
                    }
                }
            }
            ...
            {
                throw new IOException("Audit checkpoint does not match the journal; restore the complete audit directory.");
            }
    ...
    private void AppendCore(AuditRecord record)
    {
        if (poisoned)
        {
            throw new IOException("Journal requires recovery after a failed append.");
        }
        ...
        {
            throw new ArgumentException("Audit record exceeds the 16 MiB frame limit.", nameof(record));
        }
        ...
        try
        {
            ...
            {
                stream.Dispose();
                var archive = fileSystem.Path.Combine(
                    directory,
                    $"audit.{sequence.ToString("D20", System.Globalization.CultureInfo.InvariantCulture)}.archive");
                fileSystem.File.Move(fileSystem.Path.Combine(directory, "audit.journal"), archive);
                stream = fileSystem.FileStream.New(
                    fileSystem.Path.Combine(directory, "audit.journal"),
                    FileMode.CreateNew,
                    FileAccess.ReadWrite,
                    FileShare.None);
            }
    ...
    private void Track(AuditRecord record)
    {
        ...
        {
            pending[record.OperationId] = record;
        }

        if (record.Type is "Deleted" or "Failed" or "SkippedAfterRevalidation" or "Acknowledged")
        {
            pending.Remove(record.OperationId);
        }
    }
