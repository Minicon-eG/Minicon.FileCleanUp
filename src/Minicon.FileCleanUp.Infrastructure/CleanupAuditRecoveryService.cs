...
{
    public IReadOnlyList<AuditRecord> Inspect()
    {
        using var journal = new LocalAuditJournal(directory, fileSystem, timeProvider);
        return journal.GetPending();
    }

    public void Acknowledge(Guid operationId, string @operator, string reason)
    {
        using var journal = new LocalAuditJournal(directory, fileSystem, timeProvider);
        journal.Acknowledge(operationId, @operator, reason);
    }
}
