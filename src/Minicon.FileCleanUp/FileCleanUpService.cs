    ...
    IRetryJitter? retryJitter = null) : IFileCleanUpService
{
    public Task<CleanupResult> RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        return new CleanupRun(snapshot, fileSystem, timeProvider, auditJournal, logger, retryJitter ?? new RandomRetryJitter(), cancellationToken).Execute();
    }
}
...
internal sealed class CleanupRun(CleanupOptions options, IFileSystem fs, TimeProvider clock,
    ICleanupAuditJournal? providedJournal, ILogger<FileCleanUpService>? logger, IRetryJitter jitter, CancellationToken token)
{
    ...
    internal async Task<CleanupResult> Execute()
    {
        ...
        try
        {
            ...
            try { ConfigurationValidator.Validate(options, fs); }
            catch (Exception ex) when (ex is ArgumentException or NullReferenceException)
            { result.Status = CleanupStatus.InvalidConfiguration; Event("ConfigurationInvalid", 3002, ex); return result; }
            foreach (var rule in options.Rules) _ = result.StartedUtc.AddDays(-rule.RetentionDays);
            ...
            if (options.Audit.Mode == AuditMode.Required)
            {
                if (providedJournal is null)
                {
                    ...
                    journal = new LocalAuditJournal(directory, fs, clock);
                }
                else journal = providedJournal;
            ...
            {
                token.ThrowIfCancellationRequested();
                currentRule = target.Rule; currentSelector = target.Selector;
                var scope = StorageScope(target.Path);
                if (unavailableScopes.Contains(scope)) { Error(target.Path, new IOException("Storage scope unavailable.")); continue; }
                Event("RuleStarted", 1100, path: target.Path);
                try { await ProcessTarget(target); }
                catch (AuditUnavailableException) { throw; }
                catch (RetryBudgetException) { throw; }
                catch (IOException ex)
                {
                    Error(target.Path, ex);
                    if (IsTransient(ex)) { unavailableScopes.Add(scope); Event("StorageScopeUnavailable", 3203, ex, target.Path); }
                }
                catch (UnauthorizedAccessException ex) { Error(target.Path, ex); }
                Event("RuleCompleted", 1199, path: target.Path);
            }
        }
        catch (LimitReachedException) { result.LimitReached = true; if (result.Status == CleanupStatus.Succeeded) result.Status = CleanupStatus.Limited; Event("CleanupLimitReached", 3100); }
        catch (OperationCanceledException) when (token.IsCancellationRequested) { result.Status = CleanupStatus.Cancelled; Event("CleanupCancelled", 3101); }
        catch (AuditUnavailableException ex) { auditFailed = true; result.Status = CleanupStatus.Failed; Count(s => s.ErrorCount++); Event("AuditUnavailable", 3301, ex); }
        ...
        { result.Status = CleanupStatus.Failed; Count(s => s.ErrorCount++); Event("CleanupFailed", 3999, ex); }
        finally
        {
            result.CompletedUtc = clock.GetUtcNow(); result.Duration = clock.GetElapsedTime(runStart);
            result.IsPartial = result.Status != CleanupStatus.Succeeded;
            if (journal is not null && !auditFailed)
            {
                try { Audit(new() { Type = "RunCompleted", RunId = result.RunId, TimestampUtc = clock.GetUtcNow(), Reason = JsonSerializer.Serialize(result) }); }
                catch (AuditUnavailableException ex) { result.Status = CleanupStatus.Failed; result.IsPartial = true; Event("AuditUnavailable", 3301, ex); }
            }
            if (providedJournal is null) journal?.Dispose();
    ...
    private async Task<List<Target>> ResolveTargets()
    {
        ...
        foreach (var rule in options.Rules)
            ...
            {
                currentRule = rule; currentSelector = selector;
                selector.Root = fs.Path.TrimEndingDirectorySeparator(fs.Path.GetFullPath(selector.Root));
                await Read(() => { EnsureSafePath(selector.Root); return true; }, selector.Root, missingRoot: true);
                var found = 0;
                var queue = new Stack<string>();
                if (selector.Kind == SelectorKind.Path) Add(selector.Root);
                else
                {
                    ...
                    while (queue.TryPop(out var directory))
                    {
                        ...
                        {
                            if (Excluded(rule, selector, child) || await IsLink(child)) continue;
                            if (DirectoryPatterns.Matches(selector, fs.Path.GetRelativePath(selector.Root, child))) Add(child);
                            queue.Push(child);
                        }
                    }
                }
                Event(found == 0 ? "DirectorySelectionEmpty" : "DirectorySelectionCompleted", found == 0 ? 1011 : 1012, path: selector.Root);
                void Add(string path)
                {
                    found++;
                    if (targets.Any(t => t.Rule == rule && ConfigurationValidator.Comparer.Equals(t.Path, path))) return;
                    if (targets.Count >= 10000) throw new IOException("Directory selection exceeded 10000 targets.");
                    targets.Add(new(rule, selector, path)); protectedRoots.Add(path);
    ...
    {
        var stack = new Stack<(string Path, bool Finish)>(); stack.Push((target.Path, false));
        while (stack.TryPop(out var entry))
        {
            token.ThrowIfCancellationRequested();
            if (entry.Finish) { await RemoveEmpty(entry.Path, target); continue; }
            if (Blocked(entry.Path, directory: true)) { Skip(entry.Path, "AuditPending", true); continue; }
            if (!visited.Add(entry.Path)) continue;
            try
            {
                await Read(() => { EnsureSafePath(entry.Path); return true; }, entry.Path);
                ...
                if (target.Rule.RemoveEmptyDirectories) stack.Push((entry.Path, true));
                foreach (var file in files)
                {
                    token.ThrowIfCancellationRequested();
                    try { await ProcessFile(file, target); }
                    catch (AuditUnavailableException) { throw; }
                    catch (RetryBudgetException) { throw; }
                    catch (IOException ex) when (!IsTransient(ex)) { Error(file, ex); }
                    catch (UnauthorizedAccessException ex) { Error(file, ex); }
                }
                foreach (var child in children)
                {
                    if (Excluded(target.Rule, target.Selector, child) || await IsLink(child)) continue;
                    stack.Push((child, false));
                }
            }
            catch (AuditUnavailableException) { throw; }
            catch (RetryBudgetException) { throw; }
            catch (IOException ex) when (!IsTransient(ex)) { Error(entry.Path, ex); }
            catch (UnauthorizedAccessException ex) { Error(entry.Path, ex); }
        }
    }
    ...
    private async Task ProcessFile(string path, Target target)
    {
        ...
        if (Blocked(path, false)) { Skip(path, "AuditPending"); return; }
        ...
        if (!target.Rule.IncludePatterns.Any(Match) || target.Rule.ExcludePatterns.Any(Match)) { Skip(path, "Excluded"); return; }
        var attributes = await Read(() => fs.File.GetAttributes(path), path);
        if ((attributes & (FileAttributes.ReadOnly | FileAttributes.ReparsePoint)) != 0) { Skip(path, "ReadOnlyOrReparsePoint"); return; }
        ...
        if (options.DryRun)
        {
            virtuallyRemoved.Add(path); Audit(intent with { Type = "WouldDelete" }); Event("FileWouldBeDeleted", 2002, path: path, record: intent); CheckLimit(); return;
        }
        await DelayMutation();
        ...
        if (updated != observed) { Skip(path, "ChangedSinceEvaluation"); return; }
        Audit(intent);
        ...
        try
        {
            ...
            if (!deleted) { Skip(path, "ChangedSinceEvaluation"); return; }
        }
        catch (AuditUnavailableException) { throw; }
        catch (RetryBudgetException) { throw; }
        catch (UnauthorizedAccessException ex) { Audit(intent with { Type = "Failed" }); Error(path, ex); return; }
        catch (IOException ex) when ((ex.HResult & 0xffff) is 32 or 33)
        { Audit(intent with { Type = "Failed" }); Error(path, ex); return; }
        catch (IOException ex)
        {
            // A failed response does not prove that the remote delete did not happen.
            pending.Add(intent); Count(s => s.UnknownDeletionOutcomes++); incomplete.Add(path);
            Audit(intent with { Type = "OutcomeUnknown" }); Error(path, ex); Event("DeletionOutcomeUnknown", 3204, ex, path, intent); CheckLimit(); return;
        }
        try { Audit(intent with { Type = "Deleted" }); }
        catch (AuditUnavailableException) { Count(s => s.UnknownDeletionOutcomes++); throw; }
        virtuallyRemoved.Add(path); Count(s => s.FilesDeleted++); Count(s => s.DeletedFileBytes += bytes);
        Event("FileDeleted", 2001, path: path, record: intent); CheckLimit();
    }

    private async Task RemoveEmpty(string path, Target target)
    {
        if (protectedRoots.Contains(path) || Blocked(path, true) || pending.Any(p => ConfigurationValidator.Contains(path, p.Path)) || incomplete.Any(p => ConfigurationValidator.Contains(path, p))) return;
        var entries = await Read(() => fs.Directory.EnumerateFileSystemEntries(path).ToArray(), path);
        if (entries.Any(p => !options.DryRun || !virtuallyRemoved.Contains(p))) return;
        var intent = new AuditRecord
        {
            Type = "DeleteIntent",
            RunId = result.RunId,
        ...
        Audit(intent with { Type = "Deleted" }); Count(s => s.DirectoriesDeleted++); Event("DirectoryDeleted", 2101, path: path, record: intent);
    }

    private bool Blocked(string path, bool directory) => pending.Any(p => p.IsDirectory ? ConfigurationValidator.Contains(p.Path, path)
        : ConfigurationValidator.Comparer.Equals(p.Path, path));
    ...
    private void EnsureSafePath(string path)
    {
        for (var current = fs.Path.GetFullPath(path); !string.IsNullOrEmpty(current); current = fs.Path.GetDirectoryName(current))
            if ((fs.File.GetAttributes(current) & FileAttributes.ReparsePoint) != 0) throw new IOException("Reparse point in path: " + current);
    }
    private async Task DelayMutation()
    {
        token.ThrowIfCancellationRequested();
        if (attemptedMutation && options.DeleteDelayMilliseconds > 0) await Task.Delay(TimeSpan.FromMilliseconds(options.DeleteDelayMilliseconds), clock, token);
    ...
    private async Task<T> Read<T>(Func<T> operation, string path, bool missingRoot = false, bool mutation = false)
    {
        ...
        finally { if (started.HasValue) retrySeconds += clock.GetElapsedTime(started.Value).TotalSeconds; }
    }
    ...
    {
        if (path.StartsWith("\\\\")) return string.Join('\\', path.Split('\\', StringSplitOptions.RemoveEmptyEntries).Take(2));
    ...
    private void Skip(string path, string reason, bool directory = false)
    {
        ...
        if (reason == "AuditPending") { incomplete.Add(path); result.Status = CleanupStatus.CompletedWithErrors; }
        Event(directory ? "DirectorySkipped" : "FileSkipped", directory ? 2103 : 2003, path: path, reason: reason);
    }
    ...
    private void Event(string type, int id, Exception? error = null, string? path = null, AuditRecord? record = null, string? reason = null, int? attemptNumber = null, double? retryDelaySeconds = null, double? retryElapsedSeconds = null, int? exclusionIndex = null)
    {
        ...
        if (type is "FileSystemRetryScheduled" or "DirectorySelectionEmpty" or "CleanupLimitReached") level = LogLevel.Warning;
        if (type == "CleanupCompleted" && result.Status is CleanupStatus.Failed or CleanupStatus.CompletedWithErrors or CleanupStatus.InvalidConfiguration) level = LogLevel.Error;
        logger.Log(level, new EventId(id, type), properties, error, (state, _) => $"{state["EventType"]}: {state["Path"]}");
    }
    ...
    private sealed class RetryBudgetException : IOException { public RetryBudgetException() : base("Global retry budget exhausted.") { } }
}
