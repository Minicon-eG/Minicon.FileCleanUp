...
public static class ServiceCollectionExtensions
{
    public static IServiceCollection AddFileCleanUp(this IServiceCollection services, IConfigurationSection section)
    {
        ...
        return Register(services, () => section.Get<CleanupOptions>(o => o.ErrorOnUnknownConfiguration = true) ?? new());
    }
    public static IServiceCollection AddFileCleanUp(this IServiceCollection services, CleanupOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);
        return Register(services, () => JsonSerializer.Deserialize<CleanupOptions>(JsonSerializer.Serialize(options))!);
    }
    private static IServiceCollection Register(IServiceCollection services, Func<CleanupOptions> read)
    {
        services.TryAddSingleton<IFileSystem, FileSystem>();
        services.TryAddSingleton(TimeProvider.System);
        services.AddTransient<IFileCleanUpService>(sp => new ConfiguredService(read, sp.GetRequiredService<IFileSystem>(), sp.GetRequiredService<TimeProvider>(), sp.GetService<ILogger<FileCleanUpService>>()));
        return services;
    }
    private sealed class ConfiguredService(Func<CleanupOptions> read, IFileSystem fs, TimeProvider time, ILogger<FileCleanUpService>? logger) : IFileCleanUpService
    {
