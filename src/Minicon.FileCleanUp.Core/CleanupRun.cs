...
internal sealed class CleanupRun(CleanupOptions options, IFileSystem fs, TimeProvider clock,
    ICleanupAuditJournal? providedJournal, ILogger? logger, IRetryJitter jitter, CancellationToken token, Func<string, ICleanupAuditJournal> createJournal)
{
    ...
    internal async Task<CleanupResult> Execute()
    {
        ...
        try
        {
            ...
            foreach (var rule in options.Rules) _ = RetentionPolicy.Cutoff(result.StartedUtc, rule.RetentionDays);
    ...
    private async Task ProcessFile(string path, Target target)
    {
        ...
        if (!RetentionPolicy.IsExpired(observed, cutoff)) { Skip(path, "TooRecent"); return; }
