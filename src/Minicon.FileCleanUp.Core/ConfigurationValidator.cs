...
internal static class ConfigurationValidator
{
    ...
    {
        if (options.MinimumRetentionDays < 1 || options.MaxFilesToDeletePerRun < 1 || options.DeleteDelayMilliseconds < 0 || options.Resilience.MaxAttempts < 1
            || options.Resilience.InitialDelaySeconds < 1 || options.Resilience.MaxDelaySeconds < options.Resilience.InitialDelaySeconds
            || options.Resilience.MaxRetryElapsedSecondsPerOperation < 1 || options.Resilience.MaxRetryElapsedSecondsPerRun < 1 || !Enum.IsDefined(options.Audit.Mode))
            throw new ArgumentException("Invalid global options.");
        if (options.Rules.Count == 0) throw new ArgumentException("At least one rule is required.");
        var names = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var roots = new List<(string Path, string Rule)>();
        foreach (var rule in options.Rules)
        {
            if (string.IsNullOrWhiteSpace(rule.Name) || !names.Add(rule.Name) || rule.RetentionDays < options.MinimumRetentionDays || rule.Directories.Count == 0)
                throw new ArgumentException("Invalid rule name, retention or directories.");
            if (rule.RemoveEmptyDirectories && !rule.Recursive) throw new ArgumentException("RemoveEmptyDirectories requires Recursive.");
            if (rule.Timestamp != "LastWriteTimeUtc" || rule.IncludePatterns.Count == 0) throw new ArgumentException("Invalid timestamp or patterns.");
            foreach (var pattern in rule.IncludePatterns.Concat(rule.ExcludePatterns))
                if (string.IsNullOrWhiteSpace(pattern) || pattern.Contains('/') || pattern.Contains('\\')) throw new ArgumentException("File patterns must be names.");
            foreach (var selector in rule.Directories)
            {
                ...
                {
                    if (!fs.Path.IsPathFullyQualified(audit) || audit.StartsWith("\\\\") || Contains(root, fs.Path.GetFullPath(audit)))
            ...
            foreach (var selector in rule.ExcludeDirectories) ValidateSelector(selector, true);
        }
    }
    internal static bool Contains(string root, string path) => string.Equals(root, path, Comparison) || path.StartsWith(root.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar, Comparison);
    ...
    {
        if (!Enum.IsDefined(selector.Kind)) throw new ArgumentException("Unknown selector kind.");
        if (exclusion && !string.IsNullOrEmpty(selector.Root)) throw new ArgumentException("Exclusions cannot define Root.");
        if (!exclusion && selector.Kind == SelectorKind.Path)
        {
            if (selector.Pattern != null) throw new ArgumentException("Path target cannot define Pattern.");
            return;
        }
